...
void test_ast_movb_imm8_to_r2(void) {
  FILE *fp;
  vm_ast_t *ast;
  ...
  ast = vm_ast_add_child(
      ast, vm_ast_create_multi_ex_call(
               ...
               vm_ast_create_single_ex_call(VM_AST_REGISTER, "r2"),
  ...
  if (memcmp(opcode, "\x92\xfa\x0a", 3)) {
    fprintf(stderr, "[test_ast_movb_imm8_to_r2] Failed.\n");
    goto cleanup;
  }

  fprintf(stdout, "[test_ast_movb_imm8_to_r2] Success.\n");
