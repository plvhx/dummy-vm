...
#define REGS_GET_R0_VAL(regs) \
	(!regs->gp->r0->is_negative ? (regs)->gp->r0->val : VM_NEGATE_BYTE((regs)->gp->r0->val))

#define REGS_GET_R1(regs) \
	(!regs->gp->r1->is_negative ? (regs)->gp->r1->val : VM_NEGATE_BYTE((regs)->gp->r1->val))

#define REGS_GET_GP_R2(regs) \
	(!regs->gp->r2->is_negative ? (regs)->gp->r2->val : VM_NEGATE_BYTE((regs)->gp->r2->val))

#define REGS_GET_GP_R3(regs) \
	(!regs->gp->r3->is_negative ? (regs)->gp->r3->val : VM_NEGATE_BYTE((regs)->gp->r3->val))
